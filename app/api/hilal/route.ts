...
function findHilal(observer: Astronomy.Observer) {
  ...
  for (let day = 0; day <= 3; day++) {
    ...
    const age = Number(sunsetData.ageHours);
    const altitude = Number(sunsetData.altitude);
    ...
    for (let minute = 5; minute <= Math.min(25, lag - 3); minute += 1) {
...
export async function GET(request: Request) {
  try {
    ...
    const observer = new Astronomy.Observer(lat, lng, Number.isFinite(height) ? height : 0);
    ...
    if (observeTime) {
      ...
      const customData: any = moonCalc(customDate, observer, new Date(hilal.newMoonIso));

      const remainingMinutes =
        (new Date(hilal.moonsetIso).getTime() - customDate.getTime()) / 60000;

      custom = {
        ...customData,
        remainingMoonset: remainingMinutes > 0 ? remainingMinutes.toFixed(1) : "0.0",
      };
    }
